...
function BarChart({ transactions, timeframe }) {
  const chartData = useMemo(() => {
    ...
    // --- YEARLY: Shows the Year name (e.g., 2026) ---
    if (timeframe === "yearly") {
       const yearTx = transactions.filter(t => new Date(t.date).getFullYear() === now.getFullYear());
       return [{
         label: now.getFullYear().toString(),
         Inbound: getSum(yearTx, "income"),
         Outbound: getSum(yearTx, "expense")
       }];
    }
    ...
    <div style={{ width: "100%", height: 350, display: "flex", alignItems: "center", justifyContent: "center" }}>
      {!hasData ? (
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ReBarChart data={chartData} barGap={8} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
            ...
            <Bar 
              ...
              radius={[4, 4, 0, 0]} 
              barSize={timeframe === "yearly" ? 40 : 12} 
            />
            <Bar 
              name="Outbound" 
              dataKey="Outbound" 
